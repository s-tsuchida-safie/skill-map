...
async def test_get_comments(test_db, test_client):
    ...
    res = test_client.get("/articles/" + str(article_id) + "/comments", params={"offset": 1, "limit": 1})
    assert res.status_code == 200
    resBody = res.json()
    assert resBody["total"] == 3
    ...
    res = test_client.get("/articles/" + str(article_id) + "/comments", params={"offset": -1})
    assert res.status_code == 400
    assert res.json()["error_code"] == "out_of_range"

    # limitを1から100までの範囲外の値を指定してリクエストする
    res = test_client.get("/articles/" + str(article_id) + "/comments", params={"limit": 1000})
    assert res.status_code == 400
    assert res.json()["error_code"] == "out_of_range"

    res = test_client.get("/articles/" + str(article_id) + "/comments", params={"limit": 0})
    assert res.status_code == 400
    assert res.json()["error_code"] == "out_of_range"
...
async def test_post_comments(test_db, test_client):
    ...
    res = test_client.post("/articles/" + str(article_id + 100) + "/comments", json=req_body)
    assert res.status_code == 404
...
async def test_patch_comments(test_db, test_client):
    ...
    comment_id = await test_db.execute(
        query="INSERT INTO `comment` (`article_id`, `content`, `created_at`) VALUES (:article_id, :content, :created_at)",
        values={"article_id": article_id, "content": default_content, "created_at": created_at},
    )
    ...
    res = test_client.patch("/articles/" + str(article_id + 100) + "/comments/" + str(comment_id), json=req_body)
    assert res.status_code == 404

    # DB上に存在しないcomment_idでリクエストする
    req_body = {"content": new_content}
    res = test_client.patch("/articles/" + str(article_id) + "/comments/" + str(comment_id + 100), json=req_body)
    assert res.status_code == 404

    # contentを指定してリクエストする
    req_body = {"content": new_content}
    res = test_client.patch("/articles/" + str(article_id) + "/comments/" + str(comment_id), json=req_body)
    assert res.status_code == 200
    db_res = await test_db.fetch_one(
...
async def test_delete_comments(test_db, test_client):
    ...
    res = test_client.request("DELETE", "/articles/" + str(article_id + 100) + "/comments", json=req_body)
    assert res.status_code == 404

    # comment_idsを指定してリクエストする
    req_body = {"comment_ids": [comment_id]}
    res = test_client.request("DELETE", "/articles/" + str(article_id) + "/comments", json=req_body)
    assert res.status_code == 200
